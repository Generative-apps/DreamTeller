...
class DreamInterpreter:
    ...
    def start_interpretation(self, dream_text : str, user_id : str):
        user_data = get_user_data(user_id)
        if user_data is None:
            user_data = UserData(memory="", num_interpretations=1)
        elif user_data.num_interpretations > self.config['max_interpretations']:
    ...
    def finish_interpretation(self, user_id : str, answers : list[str]):
